...
def perform_guest_login(uid, password):
    try:
        resp = requests.post(
            ...
            headers={'User-Agent': "GarenaMSDK/4.0.19P9(SM-M526B ;Android 13;pt;BR;)", 'Connection': "Keep-Alive"},
...
def build_response(login_method, final_uid, final_jwt, open_id, info):
    ...
    return {
        **CREDITS,
        ...
        "Language":     lang,
        "Battle Tags":  ", ".join(t.replace("PLAYERBATTLETAGID", "") for t in battle_tags) if battle_tags else None,
...
def authorise():
    ...
    err_code = lambda msg, code, reason: (
        jsonify({**CREDITS, "status": msg, "code": code, "reason": reason}), code
    )

    # ── Path 1: JWT (fastest — decode is instant, only info fetch is async) ──
    if jwt_token:
        j_uid, _, j_region = decode_jwt_info(jwt_token)
        if not j_uid:
            return err_code("❌ Invalid JWT", 400, "Could not decode JWT token")
        info = fetch_info_by_uid(j_uid, j_region)
        return jsonify(build_response("Direct JWT", j_uid, jwt_token, None, info))

    # ── Path 2: UID + Password ────────────────────────────────────────────────
    if uid_param and password:
        acc_token, open_id = perform_guest_login(uid_param, password)
        if not acc_token:
            return err_code("❌ Invalid Credentials", 401, "UID or Password is incorrect")
        final_jwt, err = perform_major_login(acc_token, open_id)
        if not final_jwt:
            return err_code("❌ JWT Failed", 500, err or "MajorLogin returned no token")
        j_uid, _, j_region = decode_jwt_info(final_jwt)
        info = fetch_info_by_uid(j_uid, j_region)
        return jsonify(build_response("UID / Password", j_uid, final_jwt, open_id, info))

    # ── Path 3: Access Token — info fetch + openid fetch run in PARALLEL ─────
    if access_token:
        f_uid, _, f_region = get_name_region_from_reward(access_token)
        if not f_uid:
            return err_code("❌ Invalid Access Token", 400, "Could not fetch UID — token may be expired")
        ...
        if not open_id:
            info_future.cancel()
            return err_code("❌ OpenID Failed", 400, "Could not get OpenID for this UID")

        final_jwt, err = perform_major_login(access_token, open_id)
        if not final_jwt:
            return err_code("❌ JWT Failed", 500, err or "MajorLogin returned no token")

        info = info_future.result()
        return jsonify(build_response("Access Token", f_uid, final_jwt, open_id, info))

    return err_code("❌ Error", 400, "Provide JWT, UID/Password, or Access Token")
